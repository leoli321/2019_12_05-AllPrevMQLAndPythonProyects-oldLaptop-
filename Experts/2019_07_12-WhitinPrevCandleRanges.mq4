...
void OnTick()
  {
     ...
     if(AccountProfit() > TakeProfit){
      CloseAllOrders();
      if(AccountBalance() > currentAccountBalance)
         lot_size = OriginalLotSize;
         TakeProfit = OriginalTakeProfit;
         StopLoss = OriginalStopLoss;
         PreviousAccountBalanceReached = 0;
     }
     ...
     if(AccountProfit() < StopLoss){
      ...
      if(PreviousAccountBalanceReached == 0){
         currentAccountBalance = AccountBalance();
         PreviousAccountBalanceReached = 1;
      }
     }
